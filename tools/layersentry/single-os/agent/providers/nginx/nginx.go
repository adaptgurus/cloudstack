...
func(p *Provider)Plan(_ context.Context,r model.ServiceRequest,resolved string)(model.Plan,error){steps:=[]model.PlanStep{{Name:"repository",Action:"use Rocky AppStream with GPG/TLS verification"},{Name:"packages",Action:"install exact Rocky-supported nginx package "+resolved},{Name:"configure",Action:fmt.Sprintf("configure listener %s:%d",r.Network.ListenAddress,r.Network.Port)},{Name:"firewall",Action:"allow only requested CIDRs"},{Name:"service",Action:"enable and start nginx"},{Name:"health",Action:"validate systemd, nginx syntax and local HTTP health response"}};sum:=sha256.Sum256([]byte(fmt.Sprintf("%s|%s|%v",r.ServiceID,resolved,steps)));return model.Plan{ID:r.OperationID,ServiceID:r.ServiceID,Provider:p.ID(),ResolvedVersion:resolved,Digest:hex.EncodeToString(sum[:]),CreatedAt:time.Now().UTC(),Request:r,Steps:steps},nil}
func(p *Provider)Install(ctx context.Context,_ model.Operation,plan model.Plan)error{if err:=p.DNF.ValidateRepositories(ctx,repoID);err!=nil{return err};return p.DNF.InstallExactFromRepos(ctx,[]string{repoID},plan.ResolvedVersion)}
