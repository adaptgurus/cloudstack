...
func(p *Provider)Health(ctx context.Context,st model.ServiceState)(model.HealthResult,error){checks:=map[string]string{};r,err:=p.Runner.Run(ctx,"/usr/bin/systemctl","is-active","nginx.service");if err!=nil{return model.HealthResult{Healthy:false,Checks:checks,Error:err.Error()},nil};checks["systemd"]=strings.TrimSpace(r.Stdout);if _,err=p.Runner.Run(ctx,"/usr/sbin/nginx","-t");err!=nil{return model.HealthResult{Healthy:false,Checks:checks,Error:err.Error()},nil};checks["config"]="ok";client:=&http.Client{Timeout:3*time.Second,Transport:&http.Transport{DialContext:(&net.Dialer{Timeout:2*time.Second}).DialContext,DisableKeepAlives:true}};url:=fmt.Sprintf("http://%s:%d/__layersentry_health",st.Network.ListenAddress,st.Network.Port);resp,err:=client.Get(url);if err!=nil{return model.HealthResult{Healthy:false,Checks:checks,Error:err.Error()},nil};_ = resp.Body.Close();if resp.StatusCode!=http.StatusNoContent{return model.HealthResult{Healthy:false,Checks:checks,Error:"unexpected HTTP health status"},nil};checks["http"]="204";r,err=p.Runner.Run(ctx,"/usr/sbin/nginx","-v");if err!=nil{return model.HealthResult{Healthy:false,Checks:checks,Error:err.Error()},nil};checks["version"]=strings.TrimSpace(r.Stderr);return model.HealthResult{Healthy:true,Version:strings.TrimSpace(r.Stderr),Checks:checks},nil}
func(p *Provider)Upgrade(ctx context.Context,op model.Operation,plan model.Plan)error{if err:=p.Install(ctx,op,plan);err!=nil{return err};_,err:=p.Runner.Run(ctx,"/usr/bin/systemctl","reload","nginx.service");return err}
...
func(p *Provider)Backup(context.Context,model.Operation,model.ServiceState)error{return errors.New("nginx provider does not own customer application-data backup")}
func(p *Provider)Restore(context.Context,model.Operation,model.ServiceState)error{return errors.New("nginx provider does not own customer application-data restore")}
