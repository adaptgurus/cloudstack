...
func (p *Provider) Apply(ctx context.Context, op model.Operation, plan model.Plan) error {
    if p.Ansible == nil { return errors.New("Ansible runner unavailable") }
    return p.Ansible.RunPlan(ctx, "apply", op, plan)
}
func (p *Provider) Install(context.Context, model.Operation, model.Plan) error { return errors.New("install is owned by transactional Ansible Apply") }
func (p *Provider) Configure(context.Context, model.Operation, model.Plan) error { return errors.New("configure is owned by transactional Ansible Apply") }
func (p *Provider) Initialize(context.Context, model.Operation, model.Plan) error { return errors.New("initialize is owned by transactional Ansible Apply") }
func (p *Provider) Join(context.Context, model.Operation, model.Plan) error { return errors.New("cluster join is not enabled by this migrated Ansible slice") }
...
func (p *Provider) Start(ctx context.Context, op model.Operation, st model.ServiceState) error {
    if p.Spec.PackageOnly { return nil }
    if p.Ansible == nil { return errors.New("Ansible runner unavailable") }
...
func (p *Provider) Uninstall(ctx context.Context, op model.Operation, st model.ServiceState, destroy bool) error {
    if destroy { return errors.New("destructive data removal requires a separately reviewed provider operation") }
    if p.Ansible == nil { return errors.New("Ansible runner unavailable") }
