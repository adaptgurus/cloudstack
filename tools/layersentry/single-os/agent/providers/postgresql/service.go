...
func(p *Provider)Start(ctx context.Context,_ model.Operation,st model.ServiceState)error{_,err:=p.Runner.Run(ctx,"/usr/bin/systemctl","start","postgresql-"+st.ReleaseLine+".service");return err}
