...
func (p *Provider) Join(context.Context, model.Operation, model.Plan) error {
	return errors.New("apache-httpd cluster join is not supported")
}
...
func (p *Provider) Backup(context.Context, model.Operation, model.ServiceState) (model.BackupRecord, error) {
	return model.BackupRecord{}, errors.New("apache-httpd does not own customer application-data backup")
}
func (p *Provider) Restore(context.Context, model.Operation, model.ServiceState, model.BackupRecord) error {
	return errors.New("apache-httpd does not own customer application-data restore")
}
...
func (p *Provider) ResidueAudit(ctx context.Context, st model.ServiceState) (map[string]string, error) {
	...
	if _, err := os.Lstat(managedConf(st.ID)); err == nil {
...
func (p *Provider) ensureOwnerAvailable(serviceID string) error {
	...
	if strings.TrimSpace(string(raw)) != serviceID {
		return errors.New("apache-httpd is already owned by another service")
	}
	return nil
}
...
func rewritePrimaryListen(ip string, port int) error {
	...
	for i, line := range lines {
		...
		if strings.Contains(line, listenMarker) {
			if managed >= 0 {
				return errors.New("multiple LayerSentry-managed Apache Listen directives found")
			}
			managed = i
			continue
		}
		if strings.HasPrefix(trimmed, "Listen ") && !strings.HasPrefix(trimmed, "#") {
...
func restorePrimaryListen() error {
	...
	if original != "Listen 80" {
		return errors.New("stored Apache original Listen directive is invalid")
	}
	raw, err := os.ReadFile(mainConf)
	if err != nil {
		return err
	...
	if found < 0 {
		return errors.New("LayerSentry-managed Apache Listen directive is missing; refusing blind restore")
	}
	lines[found] = original
	if err = filesystem.AtomicWrite(mainConf, []byte(strings.Join(lines, "\n")), 0644, "/etc/httpd/conf"); err != nil {
		return err
	}
	if err = os.Remove(listenBackup); err != nil && !errors.Is(err, os.ErrNotExist) {
		return err
	}
	return nil
}
