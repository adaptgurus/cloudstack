...
func (p *Provider) Validate(_ context.Context, r model.ServiceRequest) error {
	if r.Category != model.CategoryApplication {
		return errors.New("tomcat category must be application")
	}
	if r.Topology != "standalone" {
		return errors.New("tomcat supports standalone topology only")
	}
	if r.ReleaseLine != "rocky9" && r.ReleaseLine != "stable" {
		return errors.New("tomcat release_line must be rocky9 or stable")
	}
	if net.ParseIP(r.Network.ListenAddress) == nil || r.Network.Port == 0 {
		return errors.New("tomcat requires an explicit guest listen IP and TCP port")
	}
	if r.Backup.Enabled || r.Backup.Schedule != "" || r.Backup.Retention != 0 {
		return errors.New("tomcat does not own deployed-application backup; backup policy must be disabled")
	}
	if len(r.Storage) != 0 {
		return errors.New("tomcat provider does not consume attached storage yet; omit storage assignments")
	}
...
func (p *Provider) Install(ctx context.Context, _ model.Operation, plan model.Plan) error {
	...
	if plan.RepositoryID != repoID || plan.RepositoryDigest == "" {
		return errors.New("tomcat plan repository provenance missing")
	}
	...
	if current != plan.RepositoryDigest {
		return errors.New("tomcat repository configuration drifted after plan confirmation")
	}
...
func (p *Provider) Join(context.Context, model.Operation, model.Plan) error {
	return errors.New("tomcat cluster join is not supported")
}
...
func (p *Provider) Health(ctx context.Context, st model.ServiceState) (model.HealthResult, error) {
	...
	r, err := p.Runner.Run(ctx, "/usr/bin/systemctl", "is-active", serviceUnit)
	if err != nil {
		return model.HealthResult{Healthy: false, Checks: checks, Error: "tomcat.service is not active"}, nil
	}
	...
	resp, err := client.Get(fmt.Sprintf("http://%s:%d/", st.Network.ListenAddress, st.Network.Port))
	if err != nil {
		return model.HealthResult{Healthy: false, Checks: checks, Error: "local Tomcat HTTP health request failed"}, nil
	}
	defer resp.Body.Close()
	_, _ = io.Copy(io.Discard, io.LimitReader(resp.Body, 4<<10))
	if resp.StatusCode < 100 || resp.StatusCode >= 500 {
		return model.HealthResult{Healthy: false, Checks: checks, Error: "Tomcat returned unhealthy HTTP status"}, nil
	}
	checks["http"] = fmt.Sprintf("%d", resp.StatusCode)
	pkg, err := p.Runner.Run(ctx, "/usr/bin/rpm", "-q", packageName)
	if err != nil {
		return model.HealthResult{Healthy: false, Checks: checks, Error: "tomcat package identity unavailable"}, nil
	}
...
func (p *Provider) Repair(ctx context.Context, _ model.Operation, _ model.Plan) error {
	if err := validateServerXML(); err != nil {
		return err
	}
	_, err := p.Runner.Run(ctx, "/usr/bin/systemctl", "restart", serviceUnit)
	return err
...
func ensureOwnerAvailable(serviceID string) error {
	...
	if strings.TrimSpace(string(raw)) != serviceID {
		return errors.New("tomcat is already owned by another service")
	}
	return nil
}
...
func rewriteConnector(ip string, port int, restore bool) error {
	...
	if restore {
		...
		if backup.Port == "" {
			return errors.New("invalid Tomcat connector backup")
		}
	}
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("parse Tomcat server.xml: %w", err)
		}
		if start, ok := tok.(xml.StartElement); ok && start.Name.Local == "Connector" && isHTTPConnector(start) {
			matches++
			if matches > 1 {
				return errors.New("multiple Tomcat HTTP connectors found; refusing ambiguous rewrite")
			}
			if restore {
				start = restoreConnector(start, backup)
			} else {
				var original connectorBackup
				start, original = manageConnector(start, ip, port)
				if _, err = os.Lstat(backupPath); errors.Is(err, os.ErrNotExist) {
					b, _ := json.MarshalIndent(original, "", "  ")
					if err = filesystem.AtomicWrite(backupPath, append(b, '\n'), 0600, "/var/lib/layersentryd/state"); err != nil {
						return err
					}
				} else if err != nil {
					return err
	...
	if matches != 1 {
		return errors.New("exactly one Tomcat HTTP connector is required")
	}
	if err = enc.Flush(); err != nil {
		return err
	}
	if err = filesystem.AtomicWrite(serverXML, out.Bytes(), 0644, "/etc/tomcat"); err != nil {
		return err
	}
	if restore {
	...
	return validateServerXML()
}
func isHTTPConnector(s xml.StartElement) bool {
	port := ""
	protocol := ""
	...
	return protocol == "" || strings.Contains(strings.ToUpper(protocol), "HTTP")
}
func manageConnector(s xml.StartElement, ip string, port int) (xml.StartElement, connectorBackup) {
	b := connectorBackup{}
	hasAddress := false
	for i, a := range s.Attr {
		...
		case "port":
			b.Port = a.Value
			s.Attr[i].Value = fmt.Sprintf("%d", port)
		case "address":
			b.Address = a.Value
			b.HadAddress = true
			hasAddress = true
	...
	if !hasAddress {
		s.Attr = append(s.Attr, xml.Attr{Name: xml.Name{Local: "address"}, Value: ip})
	}
	return s, b
}
func restoreConnector(s xml.StartElement, b connectorBackup) xml.StartElement {
	...
	if b.HadAddress && !addressRestored {
		out = append(out, xml.Attr{Name: xml.Name{Local: "address"}, Value: b.Address})
	}
	s.Attr = out
	return s
}
func validateServerXML() error {
	raw, err := os.ReadFile(serverXML)
	if err != nil {
		return err
	}
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("invalid Tomcat server.xml after rewrite: %w", err)
		}
	}
}
