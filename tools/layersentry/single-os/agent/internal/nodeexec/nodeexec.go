...
func (c Client) Run(ctx context.Context, path string, args ...string) (executor.Result, error) {
	...
	if err != nil {
		return executor.Result{}, fmt.Errorf("Node.js module helper unavailable: %w", err)
	}
	defer conn.Close()
...
func Serve(ctx context.Context, socketPath, groupName string, runner executor.Runner) error {
	...
	if runner == nil {
		return errors.New("Node.js helper runner is nil")
	}
	...
	if !fi.IsDir() || fi.Mode()&os.ModeSymlink != 0 || fi.Mode()&os.ModeSticky == 0 {
		return errors.New("unsafe Node.js helper socket directory")
	}
	if old, err := os.Lstat(socketPath); err == nil {
		if old.Mode()&os.ModeSocket == 0 {
			return errors.New("refusing non-socket Node.js helper path")
		}
...
func handle(conn *net.UnixConn, runner executor.Runner) {
	...
	if err := dec.Decode(&req); err != nil {
		_ = json.NewEncoder(conn).Encode(response{ExitCode: -1, Error: "invalid Node.js helper request"})
		return
	}
...
func validate(path string, args []string) error {
	if len(args) > 32 {
		return errors.New("too many Node.js helper arguments")
	}
	for _, a := range args {
		if len(a) > 4096 || strings.ContainsAny(a, "\x00\r\n") {
			return errors.New("unsafe Node.js helper argument")
		}
	}
	switch path {
	case "/usr/bin/dnf":
	...
	case "/usr/bin/rpm":
		if len(args) == 2 && args[0] == "-q" && args[1] == "nodejs" {
			return nil
		}
	}
	return errors.New("Node.js privileged action rejected")
}
func validateDNF(args []string) error {
	if len(args) == 4 && args[0] == "-q" && args[1] == "config-manager" && args[2] == "--dump" && args[3] == "appstream" {
		return nil
	}
	if len(args) == 5 && args[0] == "-q" && args[1] == "module" && args[2] == "repoquery" && args[3] == "--available" && args[4] == "nodejs:20" {
		return nil
	}
	if len(args) == 5 && args[0] == "-q" && args[1] == "module" && args[2] == "list" && args[3] == "--enabled" && args[4] == "nodejs" {
		return nil
	}
	if len(args) == 4 && args[0] == "-y" && args[1] == "module" && args[2] == "enable" && args[3] == "nodejs:20" {
		return nil
	}
	if len(args) == 4 && args[0] == "-y" && args[1] == "module" && args[2] == "reset" && args[3] == "nodejs" {
		return nil
	}
	if len(args) == 6 && args[0] == "-y" && args[1] == "--setopt=install_weak_deps=False" && args[2] == "--disablerepo=*" && args[3] == "--enablerepo=appstream" && args[4] == "install" && nodeNEVRA.MatchString(args[5]) {
		return nil
	}
	if len(args) == 3 && args[0] == "-y" && args[1] == "remove" && args[2] == "nodejs" {
		return nil
	}
	return errors.New("Node.js DNF grammar rejected")
}
