...
func recordAuthFailure(kind, remoteAddr string) {
	...
	if len(b.Failures) >= authMaxFailures {
		b.BlockedUntil = now.Add(authBlock)
		b.Failures = nil
	}
	authAttempts.m[key] = b
	trimAuthSourcesLocked()
...
func recordAuthSuccess(kind, remoteAddr string) {
	authAttempts.Lock()
	delete(authAttempts.m, authKey(kind, remoteAddr))
	authAttempts.Unlock()
}

func recentFailures(in []time.Time, now time.Time) []time.Time {
	cut := now.Add(-authWindow)
	out := in[:0]
	for _, t := range in {
		if t.After(cut) {
			out = append(out, t)
