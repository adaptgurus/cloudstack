...
func (s *Server) bootstrapLimited(w http.ResponseWriter, r *http.Request) {
	...
	if ok, retry := allowAuthAttempt("bootstrap", r.RemoteAddr); !ok {
		authRateLimited(w, retry)
		return
	}
	var q struct {
	...
	if err := decode(r, &q); err != nil {
		recordAuthFailure("bootstrap", r.RemoteAddr)
		bad(w, err)
		return
	}
	...
	if err != nil {
		recordAuthFailure("bootstrap", r.RemoteAddr)
		http.Error(w, "bootstrap rejected", http.StatusBadRequest)
		return
	}
...
func (s *Server) loginLimited(w http.ResponseWriter, r *http.Request) {
	...
	if ok, retry := allowAuthAttempt("login", r.RemoteAddr); !ok {
		authRateLimited(w, retry)
		return
	}
	var q struct {
	...
	if err := decode(r, &q); err != nil {
		recordAuthFailure("login", r.RemoteAddr)
		bad(w, err)
		return
	}
