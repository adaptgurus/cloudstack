...
func (s *Store) ListOperations()([]model.Operation,error){dir:=filepath.Join(s.root,"operations");entries,err:=os.ReadDir(dir);if errors.Is(err,os.ErrNotExist){return []model.Operation{},nil};if err!=nil{return nil,err};out:=make([]model.Operation,0,len(entries));for _,e:=range entries{if e.IsDir()||filepath.Ext(e.Name())!=".json"{continue};op,err:=readJSON[model.Operation](filepath.Join(dir,e.Name()));if err!=nil{return nil,fmt.Errorf("read operation %s: %w",e.Name(),err)};out=append(out,op)};sort.Slice(out,func(i,j int)bool{return out[i].UpdatedAt.After(out[j].UpdatedAt)});return out,nil}
...
func (s *Store) ListServices()([]model.ServiceState,error){dir:=filepath.Join(s.root,"state","services");entries,err:=os.ReadDir(dir);if errors.Is(err,os.ErrNotExist){return []model.ServiceState{},nil};if err!=nil{return nil,err};out:=make([]model.ServiceState,0,len(entries));for _,e:=range entries{if e.IsDir()||filepath.Ext(e.Name())!=".json"{continue};st,err:=readJSON[model.ServiceState](filepath.Join(dir,e.Name()));if err!=nil{return nil,fmt.Errorf("read service %s: %w",e.Name(),err)};out=append(out,st)};sort.Slice(out,func(i,j int)bool{return out[i].UpdatedAt.After(out[j].UpdatedAt)});return out,nil}
