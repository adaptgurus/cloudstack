...
func(m *Manager)Bootstrap(bootstrapFile,token,username,password string)error{if m.Initialized(){return errors.New("administrator already initialized")};if fi,err:=os.Lstat(m.path);err==nil&&(fi.Mode()&os.ModeSymlink!=0||!fi.Mode().IsRegular()){return errors.New("unsafe administrator record path")};expected,err:=os.ReadFile(bootstrapFile);if err!=nil{return err};want:=stringTrim(expected);if len(want)!=64||len(token)!=64||subtle.ConstantTimeCompare([]byte(want),[]byte(token))!=1{return errors.New("invalid bootstrap token")};if !usernameRE.MatchString(username)||len(password)<14||len(password)>256{return errors.New("username/password policy failed")};h,err:=bcrypt.GenerateFromPassword([]byte(password),bcryptCost);if err!=nil{return err};rec:=adminRecord{Username:username,PasswordHash:string(h),CreatedAt:time.Now().UTC()};b,err:=json.Marshal(rec);if err!=nil{return err};root:=filepath.Dir(m.path);if err=filesystem.AtomicWrite(m.path,b,0600,root);err!=nil{return err};return os.Remove(bootstrapFile)}
