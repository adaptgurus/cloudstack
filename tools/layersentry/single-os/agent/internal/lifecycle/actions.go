...
func(e *Engine)Action(ctx context.Context,serviceID string,r ActionRequest)(model.Operation,error){
 ...
 case "repair":plan:=planFromState(op,st);plan.Digest="";if plan.Digest,err=config.PlanDigest(plan);err!=nil{return fail(err,false)};if err=e.Store.SavePlan(plan);err!=nil{return fail(err,false)};op.PlanDigest=plan.Digest;_ = e.Store.SaveOperation(op);err=p.Repair(ctx,op,plan)
