...
func(e *Engine)Action(ctx context.Context,serviceID string,r ActionRequest)(model.Operation,error){
 ...
 case "repair":
  ...
  if provider.ManagesGuestPlatform(p){
   if err=verifyTransactionalProvenance(ctx,e.Runner,plan);err!=nil{return fail(err,false)}
 ...
 case "upgrade":
  req:=requestFromState(op,st,r.IdempotencyKey);resolved,e2:=p.ResolveVersion(ctx,req);if e2!=nil{return fail(e2,false)};if resolved==st.ResolvedVersion{op.Status=model.OpSucceeded;op.Stage="already-current";op.Error="";st.LastOperationID=op.ID;_ = e.Store.SaveService(st);_ = e.Store.SaveOperation(op);return op,nil};plan,e2:=p.Plan(ctx,req,resolved);if e2!=nil{return fail(e2,false)}
  if req.Network.VIP.Mode!=""{if e.VIPRunner==nil{return fail(errors.New("VIP executor unavailable for upgrade planning"),false)};plan,e2=(vip.Manager{Runner:e.VIPRunner}).AugmentPlan(ctx,req,plan);if e2!=nil{return fail(e2,false)}}
  plan.Request=req;plan.Digest="";if plan.Digest,e2=config.PlanDigest(plan);e2!=nil{return fail(e2,false)};if e2=e.Store.SavePlan(plan);e2!=nil{return fail(e2,false)};op.PlanDigest=plan.Digest;_ = e.Store.SaveOperation(op)
  if provider.ManagesGuestPlatform(p){if e2=verifyTransactionalProvenance(ctx,e.Runner,plan);e2!=nil{return fail(e2,false)}}
  err=p.Upgrade(ctx,op,plan);if err==nil{st.ResolvedVersion=resolved;st.RepositoryID=plan.RepositoryID;st.RepositoryDigest=plan.RepositoryDigest;st.PlanDigest=plan.Digest;st.PlatformPackages=append([]model.PlatformPackagePin{},plan.PlatformPackages...);st.UpdatedAt=time.Now().UTC();_ = e.Store.SaveService(st)}
