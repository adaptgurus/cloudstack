...
func(e *Engine)Action(ctx context.Context,serviceID string,r ActionRequest)(model.Operation,error){
 ...
 case "upgrade":
  ...
  plan.Request=req;plan.Digest="";if plan.Digest,e2=config.PlanDigest(plan);e2!=nil{return fail(e2,false)};if e2=e.Store.SavePlan(plan);e2!=nil{return fail(e2,false)};op.PlanDigest=plan.Digest;_ = e.Store.SaveOperation(op);err=p.Upgrade(ctx,op,plan);if err==nil{st.ResolvedVersion=resolved;st.PlanDigest=plan.Digest;st.PlatformPackages=append([]model.PlatformPackagePin{},plan.PlatformPackages...);st.UpdatedAt=time.Now().UTC();_ = e.Store.SaveService(st)}
