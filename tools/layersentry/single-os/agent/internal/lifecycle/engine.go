...
type Engine struct{Registry *provider.Registry;Store *journal.Store;Runner executor.Runner;LVMRunner executor.Runner;VIPRunner executor.Runner;Cluster *cluster.Client;LockPath string}
...
func(e *Engine)Plan(ctx context.Context,req model.ServiceRequest)(model.Plan,model.Operation,error){
 ...
 if _,err:=e.Store.GetService(req.ServiceID);err==nil{return model.Plan{},model.Operation{},errors.New("service_id already exists; allocate a new service UUID")}else if !errors.Is(err,os.ErrNotExist){return model.Plan{},model.Operation{},err}
 ...
 if op.PlanDigest!=""{plan,err:=e.Store.GetPlan(op.ID);if err!=nil{return model.Plan{},op,fmt.Errorf("load pinned plan: %w",err)};if err=validateStoredPlan(plan,op,requestDigest);err!=nil{return model.Plan{},op,err};return plan,op,nil}
 ...
 if req.Network.VIP.Mode!=""{if e.VIPRunner==nil{return model.Plan{},op,errors.New("VIP executor unavailable")};plan,err=(vip.Manager{Runner:e.VIPRunner}).AugmentPlan(ctx,req,plan);if err!=nil{return model.Plan{},op,err}}
...
func(e *Engine)Install(ctx context.Context,req model.ServiceRequest,confirmedPlanDigest string)(model.Operation,error){
 ...
 if op.RequestDigest!=requestDigest||op.IdempotencyKey!=req.IdempotencyKey||op.ServiceID!=req.ServiceID{return op,errors.New("operation request/idempotency mismatch")};if op.Status==model.OpSucceeded{return op,nil};if op.Status==model.OpUnknown{return op,errors.New("UNKNOWN operation requires authoritative reconciliation before retry")};if op.Status!=model.OpWaitingConfirmation{return op,fmt.Errorf("operation state %s is not eligible for installation",op.Status)}
 plan,err:=e.Store.GetPlan(req.OperationID);if err!=nil{return op,err};if err=validateStoredPlan(plan,op,requestDigest);err!=nil{return op,err};if confirmedPlanDigest==""||confirmedPlanDigest!=plan.Digest||confirmedPlanDigest!=op.PlanDigest{return op,errors.New("plan confirmation digest mismatch")};p,ok:=e.Registry.Get(req.Provider);if !ok{return op,errors.New("provider disappeared")}
 lk,err:=lock.Acquire(e.LockPath);if err!=nil{return op,err};defer lk.Release();current,err:=e.Store.GetOperation(op.ID);if err!=nil{return op,err};if current.Status!=model.OpWaitingConfirmation||current.PlanDigest!=op.PlanDigest{return current,errors.New("operation changed before mutation lock acquisition")};op=current
 op.Status=model.OpRunning;op.Stage="preflight-revalidation";op.Error="";_ = e.Store.SaveOperation(op);safeFail:=func(stage string,cause error)(model.Operation,error){op.Stage=stage;op.Error=redact(cause.Error());op.Status=model.OpFailedSafe;_ = e.Store.SaveOperation(op);return op,cause}
 ...
 st:=model.ServiceState{ID:req.ServiceID,Provider:req.Provider,Category:req.Category,ReleaseLine:req.ReleaseLine,ResolvedVersion:plan.ResolvedVersion,Topology:req.Topology,Storage:req.Storage,LVM:req.LVM,Network:req.Network,Maintenance:req.Maintenance,Backup:req.Backup,Cluster:req.Cluster,SecretRefs:req.SecretRefs,PlatformPackages:append([]model.PlatformPackagePin{},plan.PlatformPackages...),ConfigDigest:requestDigest,PlanDigest:plan.Digest,Status:"installing",RecoveryRequired:false,LastOperationID:op.ID,UpdatedAt:time.Now().UTC()};if err=e.Store.SaveService(st);err!=nil{return safeFail("recovery-state",err)}
 ...
 if tx,ok:=p.(provider.TransactionalGuestProvider);ok&&tx.ManagesGuestPlatform(){
  op.Stage="guest-apply";_ = e.Store.SaveOperation(op);if err=tx.Apply(ctx,op,plan);err!=nil{return fail("guest-apply",err,isAmbiguous(err))}
 }else{
  ...
  if req.Topology=="cluster"&&req.Cluster.EnrollmentPeer!=""{if e.Cluster==nil{return fail("cluster-enrollment",errors.New("cluster enrollment client unavailable"),false)};op.Stage="cluster-enrollment";_ = e.Store.SaveOperation(op);if _,err=e.Cluster.Enroll(ctx,req);err!=nil{return fail("cluster-enrollment",err,isAmbiguous(err))};op.Stage="join";_ = e.Store.SaveOperation(op);if err=p.Join(ctx,op,plan);err!=nil{return fail("join",err,isAmbiguous(err))}}else{op.Stage="initialize";_ = e.Store.SaveOperation(op);if err=p.Initialize(ctx,op,plan);err!=nil{return fail("initialize",err,isAmbiguous(err))}}
  if req.Network.Port>0{op.Stage="firewall";_ = e.Store.SaveOperation(op);if err=(firewall.Manager{Runner:e.Runner}).Apply(ctx,req.ServiceID,req.Network.Port,req.Network.AllowedCIDRs);err!=nil{return fail("firewall",err,isAmbiguous(err))}}
  if req.Network.VIP.Mode!=""{if e.VIPRunner==nil{return fail("vip",errors.New("VIP executor unavailable"),false)};op.Stage="vip";_ = e.Store.SaveOperation(op);if err=(vip.Manager{Runner:e.VIPRunner}).Apply(ctx,req.ServiceID,req.Network,plan);err!=nil{return fail("vip",err,isAmbiguous(err))}}
 ...
 op.Status=model.OpVerifying;op.Stage="health";_ = e.Store.SaveOperation(op);h,err:=p.Health(ctx,st);if err!=nil{return fail("health",err,false)};if !h.Healthy{return fail("health",errors.New(h.Error),false)}
 st.Status="installed";st.RecoveryRequired=false;st.FailureStage="";st.LastOperationID=op.ID;if err=e.Store.SaveService(st);err!=nil{return fail("commit-state",err,false)};op.Status=model.OpSucceeded;op.Stage="complete";op.Error="";_ = e.Store.SaveOperation(op);return op,nil
}
func validateStoredPlan(plan model.Plan,op model.Operation,requestDigest string)error{if plan.ID!=op.ID||plan.ServiceID!=op.ServiceID||plan.Digest==""||op.PlanDigest==""||plan.Digest!=op.PlanDigest{return errors.New("stored plan identity/digest mismatch")};canonical,err:=config.PlanDigest(plan);if err!=nil{return err};if canonical!=plan.Digest{return errors.New("stored immutable plan failed integrity verification")};d,err:=config.CanonicalDigest(plan.Request);if err!=nil{return err};if d!=requestDigest||d!=op.RequestDigest{return errors.New("stored plan request does not match operation intent")};if plan.Provider!=plan.Request.Provider||plan.ServiceID!=plan.Request.ServiceID||plan.ID!=plan.Request.OperationID{return errors.New("stored plan/request identity mismatch")};return nil}
...
func(e *Engine)Uninstall(ctx context.Context,id string,op model.Operation,destroyData bool)(model.Operation,error){st,err:=e.Store.GetService(id);if err!=nil{return op,err};p,ok:=e.Registry.Get(st.Provider);if !ok{return op,errors.New("provider unavailable")};lk,err:=lock.Acquire(e.LockPath);if err!=nil{return op,err};defer lk.Release();op.Status=model.OpRunning;op.Stage="uninstall";_ = e.Store.SaveOperation(op);if err=p.Uninstall(ctx,op,st,destroyData);err!=nil{op.Status=model.OpFailedNeedsRecovery;op.Error=redact(err.Error());st.Status="failed-needs-recovery";st.RecoveryRequired=true;st.FailureStage="uninstall";st.LastOperationID=op.ID;_ = e.Store.SaveService(st);_ = e.Store.SaveOperation(op);return op,err};if !provider.ManagesGuestPlatform(p){if e.VIPRunner!=nil&&st.Network.VIP.Mode!=""{if err=(vip.Manager{Runner:e.VIPRunner}).Remove(ctx,st.ID,st.Network);err!=nil{op.Status=model.OpFailedNeedsRecovery;op.Stage="vip-cleanup";op.Error=redact(err.Error());st.Status="failed-needs-recovery";st.RecoveryRequired=true;st.FailureStage="vip-cleanup";st.LastOperationID=op.ID;_ = e.Store.SaveService(st);_ = e.Store.SaveOperation(op);return op,err}};if e.Runner!=nil&&st.Network.Port>0{if err=(firewall.Manager{Runner:e.Runner}).Remove(ctx,id);err!=nil{op.Status=model.OpFailedNeedsRecovery;op.Stage="firewall-cleanup";op.Error=redact(err.Error());st.Status="failed-needs-recovery";st.RecoveryRequired=true;st.FailureStage="firewall-cleanup";st.LastOperationID=op.ID;_ = e.Store.SaveService(st);_ = e.Store.SaveOperation(op);return op,err}}};res,err:=p.ResidueAudit(ctx,st);if err!=nil{return op,err};for _,v:=range res{if v=="present"||v=="active"{op.Status=model.OpFailedNeedsRecovery;op.Stage="residue-audit";op.Error="managed residue remains";st.Status="failed-needs-recovery";st.RecoveryRequired=true;st.FailureStage="residue-audit";st.LastOperationID=op.ID;_ = e.Store.SaveService(st);_ = e.Store.SaveOperation(op);return op,errors.New("managed residue remains")}};st.Status="uninstalled-data-preserved";st.RecoveryRequired=false;st.FailureStage="";st.LastOperationID=op.ID;_ = e.Store.SaveService(st);op.Status=model.OpSucceeded;op.Stage="complete";op.Error="";_ = e.Store.SaveOperation(op);return op,nil}
