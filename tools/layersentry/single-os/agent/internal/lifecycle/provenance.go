...
func verifyTransactionalProvenance(ctx context.Context, runner executor.Runner, plan model.Plan) error {
    if runner == nil { return errors.New("repository provenance executor unavailable") }
    ...
    if err != nil { return fmt.Errorf("revalidate provider repository %s: %w", plan.RepositoryID, err) }
    if observed != plan.RepositoryDigest { return fmt.Errorf("provider repository %s drifted after plan confirmation", plan.RepositoryID) }
    for _, pin := range plan.PlatformPackages {
        ...
        if err != nil { return fmt.Errorf("revalidate platform repository %s: %w", pin.RepositoryID, err) }
        if observed != pin.RepositoryDigest { return fmt.Errorf("platform repository %s drifted after plan confirmation", pin.RepositoryID) }
    }
    return nil
}
