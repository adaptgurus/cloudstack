...
func validate(path string,args []string)error{if len(args)>64{return errors.New("too many LVM helper arguments")};for _,a:=range args{if len(a)>4096||strings.ContainsAny(a,"\x00\r\n"){return errors.New("unsafe LVM helper argument")}};switch path{case "/usr/sbin/pvs":return validateObserve(args,"pv");case "/usr/sbin/vgs":return validateObserve(args,"vg");case "/usr/sbin/lvs":return validateObserve(args,"lv");case "/usr/sbin/pvcreate":return validatePVCreate(args);case "/usr/sbin/vgcreate":return validateVGMutation(args,true);case "/usr/sbin/vgextend":return validateVGMutation(args,false);case "/usr/sbin/lvcreate":return validateLVCreate(args);case "/usr/sbin/mkfs.xfs":return validateMkfs(args,"-f");case "/usr/sbin/mkfs.ext4":return validateMkfs(args,"-F");case "/usr/sbin/blkid":return validateBlkid(args);case "/usr/bin/findmnt":return validateFindmnt(args);case "/usr/bin/mount":return validateMount(args)};return errors.New("LVM privileged executable rejected")}
...
func validateVGMutation(args []string,create bool)error{min:=2;if create{min=2};if len(args)<min||!lvmNameRE.MatchString(args[0]){return errors.New("VG mutation request rejected")};for _,d:=range args[1:]{if !stableDevice(d){return errors.New("VG device must use /dev/disk/by-* identity")};if err:=requireNonRootDevice(d);err!=nil{return err}};return nil}
func validateLVCreate(args []string)error{if len(args)!=7||args[0]!="-y"||args[1]!="-n"||!lvmNameRE.MatchString(args[2]){return errors.New("lvcreate grammar rejected")};if args[3]=="-L"{if !sizeRE.MatchString(args[4]){return errors.New("lvcreate size rejected")}}else if args[3]=="-l"{if args[4]!="100%FREE"{return errors.New("lvcreate extent request rejected")}}else{return errors.New("lvcreate allocation mode rejected")};if !lvmNameRE.MatchString(args[5])||args[6]!=""{return errors.New("lvcreate VG rejected")};return nil}
