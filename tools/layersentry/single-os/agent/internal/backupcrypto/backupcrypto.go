...
func load(path string) (*Keyring, error) {
	...
	for i, line := range lines {
		id, err := age.ParseX25519Identity(strings.TrimSpace(line))
		if err != nil {
...
func (k *Keyring) EncryptFile(src, srcRoot, dst, dstRoot string) (Digest, error) {
	...
	if err := prepareNewPathWithin(dst, dstRoot); err != nil {
...
func safeRegularWithin(path, root string) error {
	...
	if err := noSymlinkAncestors(root, filepath.Dir(path)); err != nil {
		return err
	}
	fi, err := os.Lstat(path)
	if err != nil {
...
func noSymlinkAncestors(root, dir string) error {
	if err := confined(dir, root); err != nil {
		return err
	}
	...
	if !rootInfo.IsDir() || rootInfo.Mode()&os.ModeSymlink != 0 || rootInfo.Mode().Perm()&0022 != 0 {
		return errors.New("backup root is unsafe")
	}
	...
	for _, part := range strings.Split(rel, string(filepath.Separator)) {
		...
		if !fi.IsDir() || fi.Mode()&os.ModeSymlink != 0 || fi.Mode().Perm()&0002 != 0 {
			return fmt.Errorf("unsafe backup ancestor %s", cur)
		}
	}
	return nil
}
