...
func ValidateIntentCapabilities(id string, r model.ServiceRequest) error {
	...
	switch id {
	case "postgresql":
		if err := allowedStoragePurposes(r, "database-data", "database-wal", "database-logs"); err != nil {
			return err
		}
		return onlySecretRefs(r, "admin_password")
	case "mysql", "mariadb":
		if err := allowedStoragePurposes(r, "database-data", "database-logs"); err != nil {
			return err
		}
		return onlySecretRefs(r, "admin_password")
	case "redis", "valkey":
		if err := allowedStoragePurposes(r, "database-data"); err != nil {
			return err
		}
		return onlySecretRefs(r, "admin_password")
	case "nginx", "apache-httpd", "tomcat":
		if err := allowedStoragePurposes(r, "application-data"); err != nil {
			return err
		}
...
func allowedStoragePurposes(r model.ServiceRequest, allowed ...string) error {
	...
	check := func(purpose string) error {
		if _, ok := set[purpose]; !ok {
			return fmt.Errorf("provider %s does not consume storage purpose %q", r.Provider, purpose)
		}
		return nil
	}
	for _, s := range r.Storage {
		if err := check(s.Purpose); err != nil {
			return err
		}
	}
	for _, g := range r.LVM {
		for _, lv := range g.LogicalVolumes {
			if err := check(lv.Purpose); err != nil {
				return err
			}
		}
	}
