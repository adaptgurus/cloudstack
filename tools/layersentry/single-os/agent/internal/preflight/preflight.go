...
func System(ctx context.Context,req model.ServiceRequest,runner executor.Runner)(Result,error){r:=Result{Checks:map[string]string{}};b,err:=os.ReadFile("/etc/os-release");if err!=nil{return r,err};s:=string(b);if !strings.Contains(s,"ID=\"rocky\"")&&!strings.Contains(s,"ID=rocky"){return r,errors.New("Rocky Linux required")};if !strings.Contains(s,"VERSION_ID=\"9")&&!strings.Contains(s,"VERSION_ID=9"){return r,errors.New("Rocky Linux 9 required")};r.Checks["os"]="rocky9";if runner==nil{return r,errors.New("preflight executor unavailable")};if out,err:=runner.Run(ctx,"/usr/sbin/getenforce");err!=nil||strings.TrimSpace(out.Stdout)!="Enforcing"{return r,errors.New("SELinux must be Enforcing")};r.Checks["selinux"]="Enforcing";if _,err:=runner.Run(ctx,"/usr/bin/systemctl","is-active","--quiet","firewalld.service");err!=nil{return r,errors.New("firewalld must be active")};if _,err:=runner.Run(ctx,"/usr/bin/systemctl","is-enabled","--quiet","firewalld.service");err!=nil{return r,errors.New("firewalld must be enabled")};r.Checks["firewalld"]="active-enabled";if b,err:=os.ReadFile("/proc/meminfo");err==nil{sc:=bufio.NewScanner(strings.NewReader(string(b)));for sc.Scan(){f:=strings.Fields(sc.Text());if len(f)>=2&&f[0]=="MemTotal:"{kb,_:=strconv.Atoi(f[1]);r.MemoryMiB=kb/1024;break}}};r.CPUs=runtimeCPUs();if r.CPUs<1{return r,errors.New("no CPU detected")};if err:=validateNetwork(req.Network);err!=nil{return r,err};if err:=validateStorage(ctx,runner,req.Storage);err!=nil{return r,err};return r,nil}
