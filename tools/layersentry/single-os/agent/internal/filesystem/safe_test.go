...
func TestEnsureUnderRejectsTraversal(t *testing.T) {
	root := t.TempDir()
	if _, err := EnsureUnder(filepath.Join(root, "..", "escape"), root); err == nil {
...
func TestAtomicWriteRoundTrip(t *testing.T) {
	...
	if err := AtomicWrite(path, []byte("safe"), 0600, root); err != nil {
		t.Fatal(err)
	}
	b, err := os.ReadFile(path)
	if err != nil || string(b) != "safe" {
		t.Fatalf("unexpected write: %q %v", b, err)
	}
}
func TestAtomicWriteRejectsSymlinkTarget(t *testing.T) {
	...
	if err := AtomicWrite(link, []byte("changed"), 0600, root); err == nil {
...
func TestAtomicWriteRejectsSymlinkParent(t *testing.T) {
	...
	if err := AtomicWrite(filepath.Join(link, "file"), []byte("x"), 0600, root); err == nil {
		t.Fatal("expected parent symlink rejection")
	}
}
func TestAtomicWriteRejectsOutsideRoot(t *testing.T) {
	root := t.TempDir()
	outside := filepath.Join(t.TempDir(), "file")
	if err := AtomicWrite(outside, []byte("x"), 0600, root); err == nil {
