...
func (c Client) Run(ctx context.Context, path string, args ...string) (executor.Result, error) {
	...
	if err != nil {
		return executor.Result{}, fmt.Errorf("Valkey helper unavailable: %w", err)
	}
	defer conn.Close()
...
func Serve(ctx context.Context, socketPath, groupName string, runner executor.Runner) error {
	...
	if runner == nil {
		return errors.New("Valkey helper runner is nil")
	}
	...
	if old, err := os.Lstat(socketPath); err == nil {
		if old.Mode()&os.ModeSocket == 0 {
			return errors.New("refusing non-socket Valkey helper path")
		}
...
func handle(conn *net.UnixConn, runner executor.Runner) {
	...
	if err := dec.Decode(&req); err != nil {
		_ = json.NewEncoder(conn).Encode(response{ExitCode: -1, Error: "invalid Valkey helper request"})
		return
	}
...
func validate(path string, args []string) error {
	if len(args) > 32 {
		return errors.New("too many Valkey helper arguments")
	}
	for _, a := range args {
		if len(a) > 4096 || strings.ContainsAny(a, "\x00\r\n") {
			return errors.New("unsafe Valkey helper argument")
		}
	}
	switch path {
	case "/usr/bin/dnf":
	...
	case "/usr/bin/rpm":
		if len(args) == 2 && args[0] == "-q" && args[1] == "valkey" {
			return nil
		}
	...
	return errors.New("Valkey privileged action rejected")
}
func validateDNF(args []string) error {
	if len(args) == 4 && args[0] == "-q" && args[1] == "config-manager" && args[2] == "--dump" && args[3] == "appstream" {
		return nil
	}
	if len(args) >= 8 && args[0] == "-q" && args[1] == "repoquery" && args[2] == "--latest-limit" && args[3] == "1" && args[4] == "--qf" && args[len(args)-1] == "valkey" {
		for _, a := range args[6 : len(args)-1] {
			if a != "--repoid=appstream" {
				return errors.New("Valkey repoquery repository rejected")
			}
		}
		return nil
	}
	...
	if len(args) == 3 && args[0] == "-y" && args[1] == "remove" && args[2] == "valkey" {
		return nil
	}
	return errors.New("Valkey DNF grammar rejected")
}
func validateSystemctl(args []string) error {
	if len(args) < 2 {
		return errors.New("Valkey systemctl action missing")
	}
	allowed := map[string]bool{"start": true, "stop": true, "restart": true, "enable": true, "disable": true, "is-active": true, "is-enabled": true}
	if !allowed[args[0]] {
		return errors.New("Valkey systemctl action rejected")
	}
	for _, a := range args[1:] {
		...
		if a != "valkey.service" {
			return errors.New("Valkey systemctl unit rejected")
		}
	}
	return nil
}
func validateRestorecon(args []string) error {
	if len(args) < 2 || len(args) > 3 || (args[0] != "-F" && args[0] != "-RF") {
		return errors.New("Valkey restorecon grammar rejected")
	}
	for _, p := range args[1:] {
		if p == "/etc/valkey/valkey.conf" || confRE.MatchString(p) {
			if args[0] != "-F" {
				return errors.New("Valkey config restorecon must be non-recursive")
			}
			continue
		}
		...
		return errors.New("Valkey restorecon path rejected")
	}
	return nil
}
func validateInstall(args []string) error {
	if len(args) == 5 && args[0] == "--directory" && args[1] == "--mode=0750" && args[2] == "--owner=valkey" && args[3] == "--group=valkey" && dataRE.MatchString(args[4]) && !strings.HasSuffix(args[4], "/dump.rdb") {
		return nil
	}
	if len(args) != 5 {
		return errors.New("Valkey install grammar rejected")
	}
	mode, owner, group, src, dst := args[0], args[1], args[2], args[3], args[4]
	switch {
	case mode == "--mode=0640" && owner == "--owner=root" && group == "--group=valkey" && strings.HasSuffix(src, "/valkey.conf") && stageRE.MatchString(src) && confRE.MatchString(dst):
		return safeExisting(src)
	case mode == "--mode=0600" && owner == "--owner=layersentry" && group == "--group=layersentry" && dataRE.MatchString(src) && strings.HasSuffix(src, "/dump.rdb") && stageRE.MatchString(dst) && strings.HasSuffix(dst, "-valkey.rdb"):
		return safeNew(dst)
	case mode == "--mode=0600" && owner == "--owner=valkey" && group == "--group=valkey" && stageRE.MatchString(src) && strings.HasSuffix(src, "-valkey-restore.rdb") && dataRE.MatchString(dst) && strings.HasSuffix(dst, "/dump.rdb"):
		return safeExisting(src)
	}
	return errors.New("Valkey install path/ownership rejected")
}
func safeParent(path string) error {
	...
	if !fi.IsDir() || fi.Mode()&os.ModeSymlink != 0 || fi.Mode().Perm()&0002 != 0 {
		return errors.New("unsafe Valkey staging directory")
	}
	return nil
}
func safeExisting(path string) error {
	...
	if fi.Mode()&os.ModeSymlink != 0 || !fi.Mode().IsRegular() || fi.Mode().Perm()&0002 != 0 {
		return errors.New("unsafe Valkey source file")
	}
	return nil
}
func safeNew(path string) error {
	if err := safeParent(filepath.Dir(path)); err != nil {
		return err
	}
	if _, err := os.Lstat(path); err == nil {
		return errors.New("Valkey destination already exists")
	} else if !errors.Is(err, os.ErrNotExist) {
		return err
	}
	return nil
