...
func NewClient(socket string) Client {
	...
	return Client{Socket: socket, Timeout: 2 * time.Minute}
}
func (c Client) Run(ctx context.Context, path string, args ...string) (executor.Result, error) {
	...
	if err != nil {
		return executor.Result{}, fmt.Errorf("data-label helper unavailable: %w", err)
	}
	defer conn.Close()
...
func Serve(ctx context.Context, socketPath, group string, runner executor.Runner) error {
	...
	if runner == nil {
		return errors.New("data helper runner nil")
	}
	...
	if !fi.IsDir() || fi.Mode()&os.ModeSymlink != 0 || fi.Mode()&os.ModeSticky == 0 {
		return errors.New("unsafe data helper socket directory")
	}
	if old, err := os.Lstat(socketPath); err == nil {
		if old.Mode()&os.ModeSocket == 0 {
			return errors.New("refusing non-socket data helper path")
		}
...
func handle(conn *net.UnixConn, runner executor.Runner) {
	...
	if err := dec.Decode(&req); err != nil {
		_ = json.NewEncoder(conn).Encode(response{ExitCode: -1, Error: "invalid data helper request"})
		return
	}
...
func validate(path string, args []string) error {
	...
	case "/usr/sbin/semanage":
		if len(args) == 5 && args[0] == "fcontext" && (args[1] == "-a" || args[1] == "-m") && args[2] == "-e" && labelSources[args[3]] && safeExternal(args[4]) {
			return nil
		}
		if len(args) == 5 && args[0] == "fcontext" && (args[1] == "-a" || args[1] == "-m") && args[2] == "-t" && args[3] == "httpd_sys_content_t" {
			pattern := args[4]
			if !strings.HasSuffix(pattern, "(/.*)?") {
				return errors.New("HTTP content SELinux pattern rejected")
			}
			if safeExternal(strings.TrimSuffix(pattern, "(/.*)?")) {
				return nil
			}
		}
		return errors.New("data SELinux labeling rejected")
	case "/usr/sbin/restorecon":
		if len(args) != 2 || args[0] != "-RF" || !safeExternal(args[1]) {
			return errors.New("data restorecon rejected")
		}
		return nil
	}
	return errors.New("data helper executable rejected")
}
func safeExternal(p string) bool {
