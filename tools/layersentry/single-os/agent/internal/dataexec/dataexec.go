...
func validate(path string,args []string)error{switch path{case "/usr/sbin/semanage":if len(args)==5&&args[0]=="fcontext"&&(args[1]=="-a"||args[1]=="-m")&&args[2]=="-e"&&labelSources[args[3]]&&safeExternal(args[4]){return nil};if len(args)==5&&args[0]=="fcontext"&&(args[1]=="-a"||args[1]=="-m")&&args[2]=="-t"&&args[3]=="httpd_sys_content_t"{pattern:=args[4];if !strings.HasSuffix(pattern,"(/.*)?"){return errors.New("HTTP content SELinux pattern rejected")};if safeExternal(strings.TrimSuffix(pattern,"(/.*)?")){return nil}};return errors.New("data SELinux labeling rejected");case "/usr/sbin/restorecon":if len(args)!=2||args[0]!="-RF"||!safeExternal(args[1]){return errors.New("data restorecon rejected")};return nil};return errors.New("data helper executable rejected")}
