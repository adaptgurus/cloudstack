...
func TestDNFAcceptsPinnedInstallShape(t *testing.T){if err:=validateDNF([]string{"-y","--setopt=install_weak_deps=False","install","nginx-1:1.24.0-1.el9.x86_64"});err!=nil{t.Fatalf("safe install rejected: %v",err)}}
