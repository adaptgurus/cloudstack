...
func Seal(p Paths)error{if !filepath.IsAbs(p.Root){return errors.New("seal root must be absolute")};customerPaths:=[]string{"state/services","operations","plans","checkpoints","secrets","evidence","backups","apps","enrollment"};if exists(filepath.Join(p.Root,"identity","admin.json")){return errors.New("refusing to seal image with initialized administrator")};for _,dir:=range customerPaths{path:=filepath.Join(p.Root,dir);entries,err:=os.ReadDir(path);if err==nil&&len(entries)>0{return fmt.Errorf("refusing to seal image with customer lifecycle state in %s",path)};if err!=nil&&!errors.Is(err,os.ErrNotExist){return err}};for _,path:=range []string{p.NodeID,p.BootstrapToken,p.TLSCert,p.TLSKey,filepath.Join(p.Root,"identity","secret.key"),filepath.Join(p.Root,"identity","backup.agekeys"),filepath.Join(p.Root,"identity","admin.json")}{if err:=os.Remove(path);err!=nil&&!errors.Is(err,os.ErrNotExist){return err}};for _,dir:=range customerPaths{path:=filepath.Join(p.Root,dir);if err:=os.RemoveAll(path);err!=nil{return err}};return nil}
