...
func Seal(p Paths)error{for _,path:=range []string{p.NodeID,p.BootstrapToken,p.TLSCert,p.TLSKey,"/var/lib/layersentryd/identity/secret.key"}{if err:=os.Remove(path);err!=nil&&!errors.Is(err,os.ErrNotExist){return err}};for _,dir:=range []string{"operations","plans","state/services","checkpoints","secrets","evidence"}{path:=filepath.Join(p.Root,dir);if err:=os.RemoveAll(path);err!=nil{return err}};return nil}
