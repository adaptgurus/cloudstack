...
func serve()error{if err:=firstBoot();err!=nil{return err};runner:=executor.OSRunner{Timeout:3*time.Minute,MaxOutput:1<<20};reg:=provider.NewRegistry();if err:=reg.Register(pgprovider.New(runner));err!=nil{return err};if err:=reg.Register(nginxprovider.New(runner));err!=nil{return err};st,err:=journal.New(root);if err!=nil{return err};sec,err:=secrets.Open(root+"/secrets",root+"/identity/secret.key");if err!=nil{return err};eng:=&lifecycle.Engine{Registry:reg,Store:st,Runner:runner,LockPath:root+"/state/mutation.lock"};authm:=auth.New(root+"/identity/admin.json");srv:=&api.Server{Engine:eng,Auth:authm,Secrets:sec,Journal:st,Registry:reg,BootstrapFile:root+"/identity/bootstrap-token",AllowedOrigin:os.Getenv("LAYERSENTRY_ALLOWED_ORIGIN")};handler,err:=srv.Handler();if err!=nil{return err};hs:=&http.Server{Addr:":9443",Handler:handler,ReadHeaderTimeout:10*time.Second,ReadTimeout:30*time.Second,WriteTimeout:5*time.Minute,IdleTimeout:60*time.Second,MaxHeaderBytes:1<<20,TLSConfig:&tls.Config{MinVersion:tls.VersionTLS12}};log.Printf("layersentryd starting HTTPS management endpoint on %s",hs.Addr);return hs.ListenAndServeTLS(root+"/identity/tls.crt",root+"/identity/tls.key")}
