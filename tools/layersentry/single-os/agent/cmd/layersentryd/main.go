...
type runtimeState struct{runner executor.OSRunner;reg *provider.Registry;store *journal.Store;secrets *secrets.Store;eng *lifecycle.Engine}
...
func buildRuntime()(*runtimeState,error){runner:=executor.OSRunner{Timeout:3*time.Minute,MaxOutput:1<<20};st,err:=journal.New(root);if err!=nil{return nil,err};sec,err:=secrets.Open(root+"/secrets",root+"/identity/secret.key");if err!=nil{return nil,err};reg:=provider.NewRegistry();if err:=reg.Register(pgprovider.New(runner,sec));err!=nil{return nil,err};if err:=reg.Register(nginxprovider.New(runner));err!=nil{return nil,err};eng:=&lifecycle.Engine{Registry:reg,Store:st,Runner:runner,LockPath:root+"/state/mutation.lock"};return &runtimeState{runner:runner,reg:reg,store:st,secrets:sec,eng:eng},nil}
func serve()error{if err:=firstBoot();err!=nil{return err};rt,err:=buildRuntime();if err!=nil{return err};authm:=auth.New(root+"/identity/admin.json");srv:=&api.Server{Engine:rt.eng,Auth:authm,Secrets:rt.secrets,Journal:rt.store,Registry:rt.reg,BootstrapFile:root+"/identity/bootstrap-token",AllowedOrigin:os.Getenv("LAYERSENTRY_ALLOWED_ORIGIN")};handler,err:=srv.Handler();if err!=nil{return err};hs:=&http.Server{Addr:":9443",Handler:handler,ReadHeaderTimeout:10*time.Second,ReadTimeout:30*time.Second,WriteTimeout:5*time.Minute,IdleTimeout:60*time.Second,MaxHeaderBytes:1<<20,TLSConfig:&tls.Config{MinVersion:tls.VersionTLS12}};log.Printf("layersentryd starting HTTPS management endpoint on %s",hs.Addr);return hs.ListenAndServeTLS(root+"/identity/tls.crt",root+"/identity/tls.key")}
func maintenanceRun()error{rt,err:=buildRuntime();if err!=nil{return err};return (maintenance.Runner{Store:rt.store,Engine:rt.eng}).Run(context.Background())}
