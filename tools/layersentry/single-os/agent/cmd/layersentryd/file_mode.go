...
func planFile(path string) error {
	...
	plan, op, err := rt.eng.Plan(context.Background(), req)
	if err != nil {
		return err
	}
...
func applyFile(path, digest string) error {
	if digest == "" {
		return errors.New("confirmed plan digest is required")
	}
	req, err := readIntentFile(path)
	if err != nil {
		return err
	...
	op, err := rt.eng.Install(context.Background(), req, digest)
	if err != nil {
		return err
	}
	enc := json.NewEncoder(os.Stdout)
...
func readIntentFile(path string) (model.ServiceRequest, error) {
	...
	if fi.Size() < 2 || fi.Size() > 1<<20 {
		return model.ServiceRequest{}, errors.New("intent file size outside 2..1048576 bytes")
	}
