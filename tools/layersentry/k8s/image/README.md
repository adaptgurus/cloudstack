...
The managed bootstrap QGA reader uses the root-owned public export `/usr/share/layersentry/node-image/ssh_host_ed25519_key.pub`. A fixed oneshot regenerates it after cloud-final on every boot; the sealed image contains no export. Original SSH public/private labels remain vendor defaults. Readers must validate ownership, regular single-link files and trusted directory ancestry and then use strict SSH to prove possession of the key. No QGA unconfined or manage-SSH SELinux boolean is enabled.
