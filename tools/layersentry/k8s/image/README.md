...
The host profile includes cloud-init, QGA/Python/SSH, iSCSI/multipath/NVMe/NFS/LVM/mdadm/cryptsetup, ext/XFS tooling and the required common diagnostics. Optional RDMA/GPU packages and workload controllers/operators are excluded. Fio supplies the initial disk-latency diagnostics; no separate ioping/yq distribution is added to the base. Storage protocols remain inactive until selected by a qualified StorageHostProfile.
