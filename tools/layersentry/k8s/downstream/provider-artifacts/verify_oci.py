...
def verify(archive, docker_inspect, component, source_commit):
    ...
    if config.get('config', {}).get('Entrypoint') != {'capc': ['/manager'], 'cloudstack-ccm': ['/app/cloudstack-ccm', '--cloud-provider', 'external-cloudstack']}[component]:
        raise VerificationError('unexpected image entrypoint')
    if config.get('config', {}).get('User') != '65532:65532':
        raise VerificationError('runtime must use the pinned non-root identity')
    attestations = {}
    for digest in reachable:
        document = blobs[digest][1]
