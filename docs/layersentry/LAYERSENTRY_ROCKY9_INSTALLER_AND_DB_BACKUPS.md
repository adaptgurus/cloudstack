...
The narrow `recover-database-bootstrap` action applies only when checkpoint and package stages are applied, the database stage is `in_progress`, MySQL is stopped, the native system schema and server identity exist, and the temporary bootstrap input is absent. It restores the reviewed loopback configuration, authenticates with the supplied administrator secret, and requires both CloudStack schemas to be absent before clearing only the database-stage marker. Any mismatch leaves the marker intact and stops/disables MySQL. A fresh preflight and normal resume remain mandatory afterward; this action never recreates or deletes a schema.
