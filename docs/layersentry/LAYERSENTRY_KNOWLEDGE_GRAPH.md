...
First-management RKE2 bootstrap uses native CloudStack VM, load-balancer, firewall and temporary port-forwarding resources. A trusted KVM host exposes the guest's public SSH identity through QGA; temporary scoped forwarding supplies reachability. The image-owned public export preserves vendor private-key labels and QGA confinement. The [native provider installer](../../tools/layersentry/k8s/management/README.md) imports an immutable bundle through that existing transport, uses local clusterctl repositories and observes exact provider inventory, CRDs and controller readiness. Verified management credentials and providers must be usable before the journal permits transport cleanup. After cleanup, inspection uses the authenticated management API and cannot silently recreate SSH. Hosted artifact qualification does not establish live provider compatibility or production certification.
...
- Package durable intent -> exact approved catalog revision + native CAPI Cluster UID -> central Flux remote Helm lifecycle; project/cluster SQLite reservation includes UNKNOWN and all cluster mutation kinds. Full-profile native resource binding permits unrelated catalog additions; historical approved catalogs preserve existing uninstall/status. Dependency readiness precedes reservation; owned packages block Cluster deletion. Evidence: `evidence/k8s/2026-09-07-package-bff-saga-source.md` (source tests only; no operator/GUI certification).

### 2026-09-07 DC prerequisite evidence

[Verified DC guest bridge and firmware mapping](evidence/dr/2026-09-07-dc-guest-network-and-firmware.json) links the exact Hyper-V sen VM ID to its distinct guest BIOS identity and the native Guest traffic label to ls-guest0. These establish network/identity prerequisites only; routing, native two-Zone recovery, authenticated GUI and Kubernetes storage survival remain separate unverified gates. [Current DR UI asset deployment](evidence/ui/2026-09-07-current-module-ui-dr-deploy.json) verifies served assets and preserved backend/configuration, not module lifecycles.

[DC served artifact verification](evidence/ui/2026-09-07-current-module-ui-dc-deploy.json) complements DR asset verification; authenticated browser and service lifecycle acceptance remain independent. [DC address and native TLS prerequisites](evidence/dr/2026-09-07-dc-address-and-tls-prerequisites.json) distinguish DHCP exclusions from existing lease revocation, Disabled Zone capacity filtering from allocation usage, and native CA certificate preparation from listener activation. The native Pod range guard is not an atomic allocation fence; the lab alternative depends on explicit administrative exclusivity and does not certify concurrent production safety.

[Native DC TLS activation evidence](evidence/dr/2026-09-07-dc-native-tls-activation.json) connects original CloudStack Root CA issuance to a protected native keystore, one journaled management restart, verified HTTPS leaf trust and authenticated native API, while preserving HTTP. Local service verification remains distinct from external firewall and browser trust validation.

[Native Pod reconciliation and modular libvirt prerequisites](evidence/dr/2026-09-07-pod-apply-and-current-lab-prerequisites.json) connect DHCP-safe management address allocation to exact native async evidence, while separating it from unknown usage and SystemVM readiness. CloudStack agent installation/setup scriptlets and modular versus monolithic libvirt authority require explicit integration review before host registration; package presence alone does not prove service topology.

[Kubernetes native login session lookup](evidence/k8s/2026-09-07-native-login-session-client.md) binds the module JSON client to CloudStack’s existing expiring ACCESS_TOKEN storage contract while preserving HttpOnly cookie security and server-side authorization. Authenticated browser acceptance must exercise this path; mocked readable cookies cannot establish normal-login integration.
