...
The SSH receiver bounds protocol output, including diagnostic replies, by the request deadline. A peer that stops reading cannot hold the catalog through an unbounded buffered error write. After expiry the receiver may close without an error frame. Treat that disconnect as an unknown transfer outcome and retry only the same sealed epoch; never trigger another capture merely because its receipt was lost. Local real-pipe regression evidence is recorded in [the integrity review](evidence/dr/2026-09-07-replication-integrity-review.md); exact Rocky SSH/backpressure validation remains required.
