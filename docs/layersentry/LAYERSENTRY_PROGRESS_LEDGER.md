...
- First image boot `34051209929` failed before guest creation because the serial file was outside libvirt's managed log path. Corrected native PTY logging allowed guest startup in `34052299028`, which then failed because the exact Rocky QGA RPC allowlist excluded `guest-exec`. Owned failed guests were removed; no passing boot is inferred.
- Read-only image audit `34053044282` established that libguestfs lacked appliance `setfiles` support and silently deferred relabeling to `/.autorelabel`; critical executables lacked SELinux xattrs. The corrected private build appliance includes policycoreutils and requires actual relabel capability, exact critical labels and no first-boot fallback. The managed image extends only the finite vendor allowlist with `guest-exec` and `guest-exec-status`; broad QGA SELinux booleans remain disabled. A root-owned post-cloud-final service exports only the public Ed25519 identity without changing private-key labels.
- Corrected image build `34054451765`, job `101543633197`, passed at `394263c14e008f264565aa2965394e4c0145da33`. Artifact `9995730547`; QCOW2 SHA `61c0b8088fc0c08a0263d8076dc6a9f2b402bc7e05becd6d7c1cde60e1a0c356`; label report SHA `edb03f5b34594c7eee3a81536ff6ad67a06f3990ae159bff9ed92330e66126c2`. All eight required vendor label types pass offline. Exact metadata is in `evidence/k8s/2026-09-06-rke2-cpu-image-relabel-build.json`. This remains unsigned, unregistered and runtime-unqualified.
- DR boot `34055287797`, job `101545992498`, runner `0d074750a969018af0e9d618c7fd372bf64f864f`, actually booted the exact image with confined QGA execution but failed waiting for its public export. Serial evidence identified a systemd ordering cycle: the exporter was installed under multi-user.target while requiring cloud-final, which itself follows multi-user.target. Owned guest `b65156fa-8bd8-415b-928c-7b902b852f5e` and its runtime workspace were removed; no retained capture fixture remains. Source `cc8abb303c` installs the exporter under cloud-init.target and preserves its post-cloud-final ordering. A native systemd graph test reproduces the old cycle and accepts the fix; all 20 image tests pass locally. Replacement image build `34056395384` binds builder `4d0d323569805ae022d6094dfa2cfa0ced0ea071` and is pending. Helper `e07a6c6f33` retains only allowlisted public readiness booleans on timeout. No guest or recovery pass is inferred.
- Native provider bundle run `34052983062` passed at `87cc2c84ef7126dc47cb44630c388c3bc02c838c`: 142 source tests, four native provider generations and all eight exact image imports/reimports. Artifact `9995125313`, bundle manifest SHA `9b9c3e0f2aeb81b42866e06e58bb9315ea5c269ddbc4a8d38ab52167fcb81fe5`. Native containerd requires `--local` to retain the intended downstream index names. Bootstrap keeps its owned temporary SSH transport until exact provider readiness; this unsigned bundle is explicitly disposable-lab-only and CCM activation remains closed.
- DC host key was independently verified against the exact trusted Hyper-V VM through public proof run `34052058332`; the automated OCR failure remains recorded and a separate lead visual receipt establishes trust. Strict pinned SSH inventory `34052767019` passed without guest configuration changes. Rocky9.8, CloudStack management/agent, libvirt/NFS, `cloudbr0` with management address, and existing `/export/primary` and `/export/secondary` were observed. No disk preparation is needed merely to recreate these existing host prerequisites.
- Fresh DC API inventory `34055138570` still reports a Disabled Basic Zone, no registered primary/image storage and no VMs. Combined read-only proof `34056232547` establishes that the existing SystemVM QCOW2 matches the official 4.22.0 KVM release asset documented for CloudStack 4.22.1.1 (SHA-256 `f2bf62b5c83fc538c01bc3b0693df6dd85dd0d0d3584c459999d70d8a436bb2a`); native registration/readiness remain unverified. The existing libvirt directory pool is unrelated to the NFS exports. Fresh pod inventory exposes a management range including known fixed .14/.20 addresses and an unrouted 10.10.20.0/24 guest range: these require reconciliation before Zone activation. B&R 401 remains the disabled-command configuration gate; optional API errors are not interpreted as absence.

Source `98b6a22f1a` adds an immutable qualified OCI package catalog and central Flux reconciliation into the exact CAPI workload cluster, including native RKE2ControlPlane kubeconfig ownership, current-generation digest readiness, dependency checks and UID/resourceVersion-fenced uninstall. Local Kubernetes tests: 163 pass with one environment-dependent Nginx skip; new source still awaits Rocky CI. Package BFF/saga integration and management Flux installation are being completed; no DBaaS/APaaS package or operator is live-qualified. Native stopped-clone source audit is recorded in `evidence/dr/2026-09-07-native-startvm-contract.md`: `startvm=false` remains supported; the volume preparation path does not start the guest.

DC/DR replication/failback and Kubernetes DBaaS/APaaS remain `PARTIAL`; native two-Zone recovery, actual management/client clusters, CSI data survival, operator lifecycle and Chrome/Firefox GUI acceptance are unfinished. No production percentage or certification is established. Work continues in isolated branches without shared integration resets or overlapping lab mutations.
...
- UI workflow `34051073958`, job `101534689571`, passed at exact source `e96e9b8d211d39bcde00704cda70b22f347d1f07`: production build, lint and **22 suites / 319 tests**. This source is not yet a live module GUI acceptance result.
