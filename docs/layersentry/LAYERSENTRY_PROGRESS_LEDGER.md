...
- Fresh DR boot run `34055287797`, runner `0d074750a969018af0e9d618c7fd372bf64f864f`, binds that new artifact and helper source `9dcd8ea01b`. It is pending networkless Rocky boot, confined QGA, public-export and fresh fixture health checks. Passing retains one exact disposable guest for the separately scoped same-host full/incremental/latest/older recovery test; no RKE2 start or production claim is implied.
...
- Fresh DC API inventory `34055138570` still reports a Disabled Basic Zone, no registered primary/image storage and no VMs. The existing on-disk SystemVM filename matches native template UUID but content/version/registration need reconciliation. B&R 401 remains the disabled-command configuration gate; optional API errors are not interpreted as absence.
