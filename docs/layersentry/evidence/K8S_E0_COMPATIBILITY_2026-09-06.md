...
Pinned CAPC `pkg/cloud/instance.go` uses `DestroyVirtualMachine` and an expunge choice derived from capabilities; `controllers/cloudstackmachine_controller.go` explicitly uses the administrative client for deletion. Follow-up source inspection confirms that `listVMInstanceDatadiskVolumeIDs` selects all attached DATADISK volumes and passes their IDs to `DestroyVirtualMachine` through `SetVolumeids`, without an ownership filter. CloudStack 4.22.1.1 `DestroyVMCmd.java` describes `volumeids` as volumes to delete, and `UserVmManagerImpl.destroyVm(DestroyVMCmd)` validates that selection, detaches disks and executes VM destruction. This source path establishes a concrete deletion hazard for externally owned disks still attached at Machine deletion; no destructive live reproduction was run.

[CAPC issue 389](https://github.com/kubernetes-sigs/cluster-api-provider-cloudstack/issues/389) reports the same CloudStack CSI data-loss scenario. Its comments show automated stale closure as not planned on 2024-12-05, not a fix. Preserve this distinction: closed issue state is not compatibility or remediation evidence. The released source inspected here still contains the unfiltered disk selection.

Consequent release gate: do not certify released CAPC 0.6.1 with CloudStack CSI or other externally owned CloudStack data disks until deletion ownership is corrected or an exact, enforced lifecycle invariant prevents them from entering that delete request, with worker replacement, remediation, controller restart and failed-drain data-retention tests. Merely disabling routine scale-down is insufficient because explicit deletion, remediation and interrupted cleanup remain possible. A future fix must retain independent disks by default when ownership is unknown; matching a mutable volume name is not adequate proof of ownership. Do not patch CloudStack core to compensate for provider ownership errors.

The second source gap concerns RKE2 registration. Pinned CAPC `pkg/cloud/isolated_network.go` creates its API load-balancer rule with private port 6443 and exposes only one control-plane endpoint port. Changing the public port does not add a backend listener on 9345. Pinned CAPRKE2 AWS/Docker examples separately configure supervisor port 9345, consistent with the [RKE2 network requirements](https://docs.rke2.io/install/requirements). The initial CAPC/RKE2 profile therefore needs an explicitly modeled, separately owned supervisor endpoint or another source-validated route; API reachability alone cannot prove automatic RKE2 join. No production endpoint design has been selected or live-tested.
...
Next evidence required: inspect exact source/CRDs for the two candidates, resolve the attached-data-disk ownership hazard, confirm ClusterClass and RKE2 control-plane/supervisor endpoint integration, research exact RKE2/CNI/CCM/CSI/Flux pins and maintained versions, review remaining high-signal open/closed issues and provider/community material, define the machine-readable release/compatibility schema, build and run local controller/envtest checks, then coordinate a separate non-DR Rocky test environment for create/scale/replace/delete and data-retention tests. No production tuple, node image, package digest or air-gap certification has been invented. DC, DR and shared Hyper-V/network/storage resources remain reserved to the dedicated DR session; this work has no live reservation or mutation.
